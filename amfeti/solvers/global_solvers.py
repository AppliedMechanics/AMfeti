...
class M_PCPGsolver(GlobalSolverBase):
    ...
    def solve(self, F_callback, residual_callback, lambda_init):
        ...
        if self._config_dict['full_reorthogonalization']:
            W = np.zeros((interface_size,interface_size), dtype=complex)  #make this array as csr
            Q = np.zeros((interface_size,interface_size), dtype=complex)
        ...
        for k in range(self._config_dict['max_iter']):
            ...
            if self._config_dict['full_reorthogonalization']:
                yk1 = zk
                for i in range(SearchDirectionCounter):
                    yki = Y[i]
                    qki = Q[i]
            ...
            # if self._config_dict['multiprecondition']:
            #
            # else :
            ...
            if self._config_dict['full_reorthogonalization']:
                Y[:,SearchDirectionCounter:SearchDirectionCounter+Fyk.shape[1]] = copy(yk)
                Q[:,SearchDirectionCounter:SearchDirectionCounter+Fyk.shape[1]] = copy(Fyk)
                SearchDirectionCounter = SearchDirectionCounter + Fyk.shape[1]
            else:
                yk_1 = copy(yk)
                qk_1 = copy(Fyk)

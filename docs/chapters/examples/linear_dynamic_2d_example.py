"""
We are going to solve a linear dynamic system using the FETI
linear dynamic solver in this example. We decided to use an aluminium
rectangular 2D structure, which is going to be fixed on the north
edge and a triangular force with a peak of 30kN, for 0.005sec.
...
input_file = '/meshes/2d_rectangle.msh'
output_path = '/results/linear_dynamic_2d_example'
...
"""
We proceed by assigning the material properties and mapping 
the global degrees of freedom for all nodes that lie on the 
Dirichlet boundary. This mapping will be used to assign the 
Dirichlet constraints later.
"""
...
"""
Then we define a triangular force that is applied from time=0.0s until 
time=0.004s with a peak of 50kn at time=0.002s and apply the Neumann 
boundary condition on the physical group of choice. Finally, we apply 
the Dirichlet conditions as well.
"""

# Neumann conditions, with the force direction [0, -1] for the [x direction, y direction]
F = triangular_force(0.0, 0.002, 0.004, 3.0e4)
...
a multicomponent mechanical system, i.e. a system consisting of substructures.
"""

# FETI-solver
substructured_system = MulticomponentMechanicalSystem(structural_composite, constant_mass=True, constant_damping=True,
                                                      all_linear=True, constraint_formulation='boolean')

"""
Since this is a linear dynamic problem, we'd like to use the LinearDynamicFetiSolver.

However, this solver requires a dictionary of integrator objects, which
are not directly provided by the MulticomponentMechanicalSystem and needs 
to be created and wrapped into that dictionary first. Such an integrator 
object describes the local dynamic behaviour. Moreover a dictionary containing 
the B matrices and starting values for the local solutions as well as their
first and second time derivatives are required. For this purpose, we need to
write a function that prepares these dictionaries, we need to pass to the 
FETI solver.
"""
...
"""
We can now use this function to define the dictionaries for the integrator 
objects, which are not directly provided by the MulticomponentMechanicalSystem 
and needs to be created and wrapped into that dictionary first. Such an 
integrator object describes the local dynamic behaviour. Moreover dictionaries 
for the B matrices, the local solutions and their first and second derivatives 
and call the linear static FETI solver. We're also passing the initial time=0.0s 
and final time=0.005s as start- and end-time to the LinearDynamicFetiSolver.
"""
...
solver.set_config({'full_reorthogonalization': True,
                   'save_history': True})
fetisolver = LinearDynamicFetiSolver(integrator_dict, B_dict, 0.0, 0.005, q_0_dict, dq_0_dict, ddq_0_dict, global_solver=solver,
...
"""
A solution object, containing all global solutions, solver-information and 
local problems, is returned by the solver.
"""
...
"""
We now have our solution, but it's a solution object so we need to read it 
out and store the solution in a way that is readable to us. We are going to 
create ``.hdf5`` and ``.xdmf`` files that contain the results.
"""

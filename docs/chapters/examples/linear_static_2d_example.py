"""
We are going to solve a linear static system using the FETI
linear static solver in this example. We decided to use an
aluminium rectangular 2D structure, which is going to be
fixed on the north edge and a force of 15kN is going to be
applied on the south edge.
...
output_path = '/results/linear_static_2d_example'
...
"""
We proceed by assigning the material properties and mapping the 
global degrees of freedom for all nodes that lie on the Dirichlet 
boundary. This mapping will be used to assign the Dirichlet 
constraints later.
"""
...
"""
Then we define the external force of 15 kN and apply the 
Neumann boundary condition. Finally, we apply the Dirichlet 
conditions as well.
"""
...
However, this AMfeti solver requires dictionaries for the K matrices, 
the B matrices and the f_ext. AMfe's MechanicalSystem only provides 
callback-functions, wo we are going to need to call these 
callback-functions with a start-displacement. For this purpose, 
we write a wrapper function that prepares these dictionaries, we need 
to pass to the FETI solver.
"""
...
"""
We can now use this function to define the dictionaries for 
K, B and f_ext and call the linear static FETI solver.
"""
...
"""
We now have our solution, but it's a solution object so we need 
to read it out and store the solution in a way that is readable 
to us. We are going to create ``.hdf5`` and ``.xdmf`` files 

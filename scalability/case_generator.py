...
def create_subdomain_mesh_components(n_domains_x,n_domains_y,n_ele_x,n_ele_y,length,height,material,force,force_direction):
...
def _create_K_B_f_dict_splitsystem(B_dict, msys_dict):
    ...
    for i_system, msys in msys_dict.items():
        ...
        K_effective_trans[subs_key] = scipy.sparse.vstack([
            scipy.sparse.hstack([K_dict_trans[subs_key]-omg2 * M_dict_trans[subs_key], omg*(aa * K_dict_trans[subs_key] + bb * M_dict_trans[subs_key])]),
            scipy.sparse.hstack([K_dict_trans[subs_key]-omg2 * M_dict_trans[subs_key], omg*(aa * K_dict_trans[subs_key] + bb * M_dict_trans[subs_key])]) ])
    ...
    for key, items in B_dict.items():
        ...
        B_dict_split[key] = scipy.sparse.coo_matrix( (items_coo.data,(items_coo.row,items_coo.col)),shape=(items_coo.shape[0]*2,items_coo.shape[1]*2)  )
...
def create_subdomain_matrices(n_domains_x=2, n_domains_y=2, n_ele_X_global=10, n_ele_Y_global=10, length=8, height=8, split_system = False):
    logging.info('Test to see where the logging is written')
    E_alu = 210e9
    nu_alu = 0.3
    rho_alu = 7850
    material = KirchhoffMaterial(E_alu, nu_alu, rho_alu, thickness=1.0)

    n_ele_X_subdomain = n_ele_X_global/n_domains
    n_ele_Y_subdomain = n_ele_Y_global/n_domains

    force           = constant_force(100000)
    force_direction = np.array([0.0, 1.0])
    mesh_dict,components_ids_list,components_list = create_subdomain_mesh_components(n_domains_x,n_domains_y,n_ele_X_subdomain,n_ele_Y_subdomain,length,height,material,force,force_direction)
    ...
    substructured_system_components = MulticomponentMechanicalSystem(structural_composite_components,'boolean',all_linear=True)
    substructured_system = substructured_system_components
    structural_composite = structural_composite_components

    if split_system:
        K_dict, M_dict, B_dict, f_dict, K_effective_dict, f_effective_dict =_create_K_B_f_dict_splitsystem(substructured_system.connections, substructured_system.mechanical_systems)
    else:
        K_dict, M_dict, B_dict, f_dict, K_effective_dict, f_effective_dict  =_create_K_B_f_dict(substructured_system.connections, substructured_system.mechanical_systems)

    return K_dict, M_dict, B_dict, f_dict, K_effective_dict, f_effective_dict
